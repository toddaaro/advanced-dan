...
;;; A reversible scheme interpreter that finds quines
;;; Aaron Todd
...
;; this finds the first quine, and calls eval on it
;; then the result is check for equality against the first quine
;; the cars are there because mk returns lists of results
...
(define map-valo
  (lambda (lst env res)
    (conde
      [(== '() lst) (== res '())]
      [(fresh (fst rest fst-c rest-c)
         (conso fst rest lst)
         (conso fst-c rest-c res)
         (valo fst env fst-c)
         (map-valo rest env rest-c)
        )])))
...
(define valo
  (lambda (exp env val)
    (matche `(,exp ,env ,val)
     ...
     [((list . ,elems) ,env ,val) (map-valo elems env val)]

     [(,exp ,env ,val)
      (never-pairo exp)
      (apply-envo env exp val)]
          
     [((lambda (,x) ,body) ,env ,val)
      (== val (closureo x body env))]

     [((,rator ,rand) ,env ,val)
      (fresh (proc arg p-env body x new-env qi qh)
        (valo rator env proc)
        (== proc `(closure ,x ,body ,p-env))
        (valo rand env arg)
        (extend-envo x arg p-env new-env)
        (valo body new-env val)
        )]
     )))
