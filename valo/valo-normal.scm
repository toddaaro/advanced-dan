...
(define closureo
  (lambda (x body env closure)
    (== closure `(,x ,body ,env))))

(define apply-proco
  (lambda (closure arg val)
    (fresh (x body env new-env)
      (== closure `(,x ,body ,env))
  ...
  (lambda (exp env val)
    (matche `(,exp ,env ,val)
      ...
      [((lambda (,x) ,body) ,env ,val)
       (closureo x body env val)]

      [((,rator ,rand) ,env ,val)
       (fresh (proc arg)
         (valo rator env proc)
         (valo rand env arg)
         (apply-proco proc arg val))]
...
(define run-tests
  ...
  (test-check "if-inside-lambda"
    (run* (q)
      (valo '((lambda (x) (if (zero? (var x)) (1) (0 1))) ()) '() q))
    '((1)))

  (test-check "lambda-1"
    (run* (q)
      (valo '(lambda (x) (var x)) '() q))
    '((x (var x) ())))

  (test-check "lambda-2"
    (run* (q)
      (valo '((lambda (x) (var x)) (0 1)) '() q))
    '((0 1)))

  (test-check "lambda-3"
    (run* (q)
      (valo '((lambda (x) (* (0 1) (sub1 (var x)))) (1 1)) '() q))
    '((0 0 1)))
 
  (test-check "self-passing-fact"
    (run* (q)
      (valo '((lambda (f) (((var f) (var f)) (1 0 1)))
              (lambda (f)
