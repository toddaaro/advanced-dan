...
;;; A lambda-calculus normal-form evaluator in miniKanren
  ...
  (lambda (l1 l2 u)
    (matche `(,l1 ,l2 ,u)
      ...
      [((,h . ,t) ,l2 ,u)
       (fresh (new-l2 res-l2)
         (rembero h l2 new-l2)
         (appendo `(,h) new-l2 res-l2)
         (uniono t res-l2 u))])))

;;; Normal-form evaluator
  ...
  (lambda (exp x res)
    (matche `(,exp ,x ,res)
      ...
      [((lambda (,a) ,body) ,x ,res)
       ...
       (freeo body x res)]
      [((,rator ,rand) ,x ,res)
       (fresh (rator-res rand-res)
         (freeo rator x rator-res)
         (freeo rand x rand-res)
         (oro rator-res rand-res res))])))

;; Makes a substitution according to the rules of safe substitutions
(define safe-substitute
  (lambda (exp pre post result)
    (matche `(,exp ,pre ,post ,result)
      ...
      [((,rator ,rand) ,pre ,post ,result)
       (fresh (rator-subs rand-subs)
         (== result `(,rator-subs ,rand-subs))
         (safe-substitute rator pre post rator-subs)
         (safe-substitute rand pre post rand-subs))]

      [((lambda (,x) ,body) ,pre ,post ,result)
       (never-pairo x)
       (== x pre)
       (== result `(lambda (,x) ,body))]

      [((lambda (,x) ,E) ,v ,E1 ,result)
       (fresh (Es)
         (freeo E1 x #f)
         (=/= x v)
         (safe-substitute E v E1 Es)
         (== result `(lambda (,x) ,Es)))]
      ...
      [((lambda (,x) ,E) ,v ,E1 ,result)
       (fresh (z Es Ess)
         ...
         (freeo E1 z #f)
         (=/= z v)
         (safe-substitute E x z Es)
         (safe-substitute E v E1 Ess)
         (== result `(lambda (,z) ,Ess)))]

      )))

;; The results are possible outcomes of applying one redex
(define redex
  (lambda (e red)
    (matche `(,e ,red)
      [(,x ,x) (never-pairo x)]
...
;; Result is the normal form of the expression
(define nfeo
  (lambda (e nf)
    (fresh (red)
      (redex e red)
