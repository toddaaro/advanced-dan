...
;;; Converts lambda-calculus into the SK basis
...
(define abstracto
  (lambda (exp res)
    (matche `(,exp ,res)
      [((lambda (,x) ,body) ,res)
       (abstracto-inner x body res)]
      [((,rator ,rand) (,rator^ ,rand^))
       (abstracto rator rator^)
       (abstracto rand rand^)])))

(define abstracto-inner
  (lambda (x body res)
    (matche `(,x ,body ,res)
      [(,x ,x ((S K) K))]
      [(,x ,y (K ,z))
       (abstracto y z)]
      [(,x (,rator ,rand) ((S ,rand^) ,rator^))
       (abstracto-inner x rator rator^)
       (abstracto-inner x rand rand^)]
      [(,x (lambda (,y) ,body2) ,res)
       (fresh (body2^)
         (abstracto-inner body2 y body2^)
         (abstracto body2^ res))])))

(define inc-vs
  (lambda (ls)
    (map (lambda (e)
           (list (car e) (+ (cadr e) 1))) ls)))

(define new-v
  (lambda (x ls)
   (cons (list x 0) (filter (lambda (z) (not (eq? (car z) x))) ls))))
...
(define lex
  (lambda (e)
    (letrec
        ([lexx (lambda (ee vs)
                 (pmatch ee
                   [,x (guard (symbol? x) (assoc x vs)) (car (cdr (assoc x vs)))]
                   [,x (guard (symbol? x)) (list 'free-var x)]
                   [(lambda (,x) ,M) `(lambda ,(lexx M (update-vs x vs)))]
                   [(,rator ,rand) `(,(lexx rator vs) ,(lexx rand vs))]))])
      (lexx e '()))))


;; (trace-define-mk labstracto
;;   (lambda (exp res)
;;     (matche `(,exp ,res)
;;       [((lambda ,body) ,res)
;;        (labstracto-inner body res)]
;;       [((,rator ,rand) (,rator^ ,rand^))
;;        (labstracto rator rator^)
;;        (labstracto rand rand^)])))

;; (trace-define-mk labstracto-inner
;;   (lambda (body res)
;;     (matche `(,body ,res)
;;       [(0 ((S K) K))]
;;       [(,n (K ,z))
...
;;        (labstracto n z)]
;;       [(,x (,rator ,rand) ((S ,rand^) ,rator^))
;;        (labstracto-inner rator rator^)
;;        (labstracto-inner rand rand^)]
;;       [((lambda ,body2) ,res)
;;        (fresh (body2^)
;;          (labstracto-inner body2 body2^)
;;          (labstracto body2^ res))])))

(define parse
  (lambda (exp)
    (let ([exp (lex exp)])
      (letrec ([parse (lambda (exp)
                        (pmatch exp
                          [,x (guard (number? x)) (build-num x)]
                          [(lambda ,body) `(lambda ,(parse body))]
